...
INSTANCEID=$(curl -sq http://169.254.169.254/latest/meta-data/instance-id)
#ex: us-east-1a => us-east-1
REGION=$${REGION: :-1}
aws ec2 associate-address --instance-id $${INSTANCEID} --allocation-id ${eip_id} --region $${REGION}
...
LOG_DIR="/var/log/bastion/"
REGION="${logs_bucket_region}"
#ex: us-east-1a => us-east-1
REGION=$${REGION: :-1}
aws s3 cp $LOG_DIR s3://${logs_bucket_name}/logs_bucket_prefix/ --sse --region $REGION --recursive && find $LOG_DIR* -mtime +1 -exec rm {} \;
...
aws s3api list-objects --bucket ${keys_bucket_name} --prefix ${keys_bucket_prefix}/ --region $REGION --output text --query 'Contents[?Size>`0`].Key' | sed -e 'y/\t/\n/' > ~/keys_retrieved_from_s3
while read line; do
  ...
  if [[ "$USER_NAME" =~ ^[a-z][-a-z0-9]*$ ]]; then
    ...
    if [ -f ~/keys_installed ]; then
      ...
      if [ $? -eq 0 ]; then
        aws s3 cp s3://${keys_bucket_name}/${keys_bucket_prefix}/$line /home/$USER_NAME/.ssh/authorized_keys --region $REGION
